...
async def start(msg: types.Message):
    ...
    for code, label in [("he","🇮🇱 עברית"),("en","🇬🇧 English"),("ru","🇷🇺 Русский"),("ar","🇸🇦 العربية"),("fr","🇫🇷 Français"),("es","🇪🇸 Español"),("zh","🇨🇳 中文"),("pt","🇧🇷 Português")]:
...
# ================== BROADCAST MESSAGE (ready for Osif) ==================
@dp.message_handler(commands=['ready_broadcast'])
async def ready_broadcast(msg: types.Message):
    if msg.from_user.id != ADMIN_ID: await msg.answer("⛔ Admin only"); return
    text = """🌟 *NIFTI  הכרטיס ביקור החכם הראשון שמרוויח לך כסף!* 🌟

👋 היי, כאן אוסיף אונגר,
אני מזמין אותך להצטרף למהפכה! יצרתי את NIFTI כי איבדתי את חשבון הבנק שלי, ונאלצתי להמציא את עצמי מחדש. אני לא רוצה שאף אחד יעבור את מה שאני עברתי.

NIFTI נותן לך:
✅ כרטיס ביקור דיגיטלי חכם  תמיד בתוקף, תמיד מעודכן
✅ קישור אישי לשיתוף  שתף בוואטסאפ, לינקדאין, טלגרם
✅ מנגנון הפניות  קבל 51% עמלה על כל מי שיוצר כרטיס דרכך
✅ בלי בנק  רק ארנק TON (קריפטו). הכסף נכנס ישירות אליך!
✅ מעל 200 כרטיסים בחינם לנרשמים הראשונים (קוד: NIFTI200)

🎯 *איך מתחילים?*
1. לחץ כאן: @NFTY_madness_bot
2. בחר שפה ולחץ "צור כרטיס חינם"
3. מלא שם, מקצוע, ארנק TON
4. שתף את הקישור שלך  תתחיל להרוויח!

💡 *אין לך ארנק TON?* 
פתח ארנק Tonkeeper (חינם) או שלח אליי ואסביר.

בואו נבנה עתיד שבו כל אחד יכול להתפרנס, בלי תלות באף אחד.

נ.ב.  יש לי 200 כרטיסים חינם למהירים! ⏳"""
