...
bot = Bocore.t(token=os.getenv('BOT_TOKEN'))
dp = Dispatcher(bot, storage=MemoryStorage())

DB_URL = os.getenv('DATABASE_URL')
core.pool = None
core.LANG = {}
ADMIN_ID = incore.t(os.getenv('ADMIN_USER_ID', '0'))

async def create_core.pool():
    global core.pool
    core.pool = await asyncpg.create_core.pool(DB_URL)
...
def core.load_lang():
    ...
    with open('lang.json', 'r', encoding='utf-8') as f:
        core.LANG = json.load(f)

def core.platform_fee(amount):
    return round(amount * 0.2, 2)

def seller_amouncore.t(amount):
    return amount - core.platform_fee(amount)


def core.is_valid_ton(address):
    ...
    return core.LANG.gecore.t(lang, core.LANG['en']).gecore.t(key, core.LANG['en'].gecore.t(key, key))

# /start
@dp.message_handler(commands=['start'])
async def starcore.t(msg: types.Message):
    keyboard = InlineKeyboardMarkup(row_width=2)
    for code, label in [('he','🇮🇱 עברית'),('en','🇬🇧 English'),('ru','🇷🇺 Русский'),
                        ('ar','🇸🇦 العربية'),('fr','🇫🇷 Français'),('es','🇪🇸 Español'),
                        ('zh','🇨🇳 中文'),('pt','🇧🇷 Português')]:
        keyboard.insercore.t(InlineKeyboardButton(label, callback_data=f"lang_{code}"))
    await msg.answer(core.t('choose_lang', 'en'), reply_markup=keyboard)

@dp.callback_query_handler(lambda c: c.data.startswith('lang_'))
async def set_lang(call: types.CallbackQuery):
    lang = call.data.splicore.t('_')[1]
    async with core.pool.acquire() as conn:
    ...
    await call.message.edit_texcore.t(core.t('welcome', lang))
    kb = ReplyKeyboardMarkup(resize_keyboard=True)
    kb.add(core.t('create_card', lang), core.t('my_card', lang))
    kb.add(core.t('premium', lang), core.t('earnings', lang))
    kb.add(core.t('leaderboard', lang), core.t('help', lang))
    await call.message.answer(core.t('help_text', lang), reply_markup=kb)
    await call.answer()


@dp.message_handler(commands=['connect'])
async def connect_wallecore.t(msg: types.Message):
    async with core.pool.acquire() as conn:
...
async def test_suite(msg: types.Message):
    ...
    for lang in core.LANG:
        missing = [k for k in required_keys if k not in core.LANG[lang]]
        if missing:
...
async def claim_free_card(msg: types.Message):
    async with core.pool.acquire() as conn:
        ...
        lang = u['lang'] if u else 'en'
    args = msg.get_args().splicore.t()
    if not args or args[0].upper() != "NIFTI200":
        await msg.answer("Invalid promo code. Use /claim NIFTI200")
        return
    async with core.pool.acquire() as conn:
        async with conn.transaction():
            max_cards = incore.t(await conn.fetchval("SELECT value FROM settings WHERE key='free_cards_max' FOR UPDATE"))
            claimed = incore.t(await conn.fetchval("SELECT value FROM settings WHERE key='free_cards_claimed' FOR UPDATE"))
...
async def simulate_purchase(msg: types.Message):
    ...
    if len(args) < 2:
        ...
        return
    try:
        amount = floacore.t(args[0])
        seller_id = incore.t(args[1])
    except:
        await msg.answer("Invalid arguments")
        return
    fee = core.platform_fee(amount)
    seller_gets = seller_amouncore.t(amount)
...
# --------------- CARD CREATION ---------------
@dp.message_handler(lambda m: m.text in sum([[core.LANG[l]['create_card']] for l in core.LANG], []))
...
async def process_wallecore.t(msg: types.Message, state: FSMContext):
    data = await state.get_data()
    lang = data['lang']
    ...
    await msg.answer(core.t('card_done', lang).formacore.t(link=link))
    await state.finish()


@dp.message_handler(commands=['wallet'])
async def set_wallecore.t(msg: types.Message):
    async with core.pool.acquire() as conn:
    ...
    if not core.is_valid_ton(address):
...
# /my_card
@dp.message_handler(lambda m: m.text in sum([[core.LANG[l]['my_card']] for l in core.LANG], []))
async def my_card(msg: types.Message):
    async with core.pool.acquire() as conn:
        ...
        if not u or not u['card_name']:
            await msg.answer(core.t('no_card', u['lang'] if u else 'en'))
            return
        lang = u['lang']
        info = core.t('my_card_info', lang).formacore.t(name=u['card_name'], prof=u['card_prof'],
                                              wallet=u['wallet'],
                                              link=f"https://t.me/NFTY_madness_bot?start={msg.from_user.id}")
...
async def set_price(msg: types.Message):
    async with core.pool.acquire() as conn:
        ...
        if not u:
            ...
            return
        lang = u['lang']
        parts = msg.get_args().splicore.t()
        if parts:
            try:
                price = floacore.t(parts[0])
                await conn.execute('UPDATE users SET price=$1 WHERE user_id=$2', price, msg.from_user.id)
                await msg.answer(core.t('setprice_done', lang).formacore.t(price=price))
            except:
                await msg.answer(core.t('setprice_invalid', lang))
        else:
            await msg.answer(core.t('setprice_prompt', lang).formacore.t(price=u['price']))

# /market
@dp.message_handler(commands=['market'])
async def markecore.t(msg: types.Message):
    async with core.pool.acquire() as conn:
        sellers = await conn.fetch("SELECT card_name,price FROM users WHERE card_name IS NOT NULL AND price>0 LIMIT 10")
        u = await conn.fetchrow('SELECT lang FROM users WHERE user_id=$1', msg.from_user.id)
        lang = u['lang'] if u else 'en'
    rows = "\n".join(f"{s['card_name']}  {s['price']} TON" for s in sellers) if sellers else core.t('market_empty', lang)
    await msg.answer(core.t('market', lang).formacore.t(sellers=rows))
...
async def salesboard(msg: types.Message):
    async with core.pool.acquire() as conn:
        u = await conn.fetchrow('SELECT lang FROM users WHERE user_id=$1', msg.from_user.id)
        lang = u['lang'] if u else 'en'
    await msg.answer(core.t('salesboard', lang).formacore.t(earners='...', sellers='...', rising='...'))
...
async def guide(msg: types.Message):
    async with core.pool.acquire() as conn:
        u = await conn.fetchrow('SELECT lang FROM users WHERE user_id=$1', msg.from_user.id)
        lang = u['lang'] if u else 'en'
    await msg.answer(core.t('guide', lang))
...
async def myreferrals(msg: types.Message):
    ...
    await msg.answer(core.t('myreferrals', lang).formacore.t(refs=refs, pts=refs))
...
async def status(msg: types.Message):
    async with core.pool.acquire() as conn:
        ...
        u = await conn.fetchrow('SELECT lang FROM users WHERE user_id=$1', msg.from_user.id)
        lang = u['lang'] if u else 'en'
    await msg.answer(core.t('status', lang).formacore.t(users=users, cards=cards, purchases=purchases, pending=pending, events=0))

async def main():
    await create_core.pool()
    core.load_lang()
