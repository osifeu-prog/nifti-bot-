...
async def cmd_status(message: Message):
    ...
    async with pool.acquire() as conn:
        ...
        purchases_today = await conn.fetchval("SELECT COUNT(*) FROM purchases WHERE created_at::date = $1", today)
...
async def demo_test(message: Message):
    user_id = message.from_user.id
    pool = await get_pool()
    async with pool.acquire() as conn:
        product = await conn.fetchrow("SELECT * FROM premium_products LIMIT 1")
        if not product:
            await message.answer("אין מוצרים להדגמה.")
            return
        invoice_id = str(uuid.uuid4())
        await conn.execute("""
            INSERT INTO purchases (buyer_user_id, product_id, referrer_user_id, amount_ton, invoice_id, status, tx_hash)
            VALUES ($1, $2, NULL, $3, $4, 'paid', 'demo_tx')
        """, user_id, product["id"], product["price_ton"], invoice_id)
        await conn.execute("""
            INSERT INTO commissions (from_user_id, to_user_id, amount_ton, level, status, purchase_id)
            VALUES ($1, $2, $3, 1, 'paid', (SELECT id FROM purchases WHERE invoice_id = $4))
        """, user_id, ADMIN_USER_ID, product["price_ton"], invoice_id)
    await message.answer("✅ דמו הושלם! הרכישה נרשמה (ללא תשלום אמיתי). בדוק /status.")
