import asyncio, os, logging, traceback, re, time
    ...
    if sellers:
        ...
        for s in sellers:
            try:
                ...
                kb.insert(InlineKeyboardButton(label, callback_data=f"buy_{s['card_name']}_{s['price']}"))
            except Exception as e:
                logging.error(f"Buy button error: {e}")
        await msg.answer(core.t("market", lang).format(sellers=rows), reply_markup=kb)
    else:
